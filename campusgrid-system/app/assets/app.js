...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    else if (location.pathname.includes('mydata')) await loadMyData();
  } catch (err) {
...
async function loadAssistant() {
  ...
  const syncBanner = $('.sync-banner span:last-child') || $('.sync-banner span:not(.sync-ic)');
  if (syncBanner && tt.timetable) {
    syncBanner.textContent = `Schedule synced · ${tt.timetable.class_count} classes · stored under your account`;
  } else if (syncBanner) {
    syncBanner.textContent = 'No timetable yet — tap Upload below (CSV or ICS)';
  }

  const uploadTitle = $('.upload-title');
  const uploadSub = $('.upload-sub');
  if (uploadSub && tt.timetable) {
    if (uploadTitle) uploadTitle.textContent = 'Timetable on server';
    uploadSub.textContent = `${tt.timetable.filename} · ${tt.timetable.class_count} classes, ${tt.timetable.location_count} locations`;
  }

  const firstBot = $('.bubble.bot');
  if (firstBot && user?.name) {
    firstBot.textContent = `Hi ${user.name} — upload your timetable, then ask when to leave. I'll use distance and average walk time from the grid.`;
  }
  if (tt.guidance?.text) {
    const g = document.createElement('div');
    g.className = 'bubble bot reveal';
    g.textContent = tt.guidance.text;
  ...
  if (uploadCard) {
    ...
    fileInput.addEventListener('change', async () => {
      ...
      try {
        ...
        if (r.guidance?.text) {
          addBubble(r.guidance.text, 'bot', r.guidance.route, r.guidance.route_dest, r.guidance.route_href, r.guidance.route_mode);
        }
        showToast('Timetable saved to your account');
      } catch (e) { showToast(e.message); }
    });
  }
...
// ---- My Data hub (all items for one user) ----
async function loadMyData() {
  const data = await CampusAPI.hub();
  const u = data.user || {};
  $('#hubName').textContent = u.name || '—';
  $('#hubMatrix').textContent = u.matrix_id || u.email || '—';
  $('#hubRfid').textContent = u.rfid_tag || '—';
  $('#hubFace').textContent = u.face_file ? 'Saved' : 'Not uploaded';
  const img = $('#hubFaceImg');
  if (img && u.face_file) {
    img.src = '/faces/' + encodeURIComponent(u.face_file);
    img.style.display = 'block';
  }

  const guide = $('#hubGuidance');
  if (guide) {
    guide.textContent = data.guidance?.text
      || 'No timetable yet. Upload CSV/ICS below, then ask the assistant when to leave.';
  }

  const ttSub = $('#hubTtSub');
  if (ttSub && data.timetable) {
    ttSub.textContent = `${data.timetable.filename} · ${data.timetable.class_count} classes, ${data.timetable.location_count} locations`;
  }

  const classBox = $('#hubClasses');
  if (classBox) {
    const days = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
    classBox.innerHTML = (data.classes || []).map(c => `
      <div class="list-item" style="pointer-events:none;">
        <div class="li-icon">▣</div>
        <div class="li-body">
          <div class="li-title">${esc(c.name)}</div>
          <div class="li-sub">${esc(c.location_name || c.location_slug)} · ${esc(c.start_time)}–${esc(c.end_time)}${c.day_of_week != null ? ' · ' + days[c.day_of_week] : ''}</div>
        </div>
      </div>`).join('') || '<div class="card dim-card"><div class="dim-text">No classes stored yet.</div></div>';
  }

  function renderDistances(records, total) {
    $('#hubDistTotal').textContent = `${total ?? 0} km`;
    const box = $('#hubDistances');
    if (!box) return;
    box.innerHTML = (records || []).map(r => `
      <div class="list-item" style="pointer-events:none;">
        <div class="li-icon">↝</div>
        <div class="li-body">
          <div class="li-title">${r.distance_km} km</div>
          <div class="li-sub">${esc([r.from_label, r.to_label].filter(Boolean).join(' → ') || r.note || 'Recorded')} · ${fmtTime(r.recorded_at)}</div>
        </div>
      </div>`).join('') || '<div class="card dim-card"><div class="dim-text">No recorded distances yet — add one above anytime.</div></div>';
  }
  renderDistances(data.distances, data.distance_total_km);

  const att = $('#hubAttendance');
  if (att) {
    att.innerHTML = (data.attendance || []).slice(0, 8).map(a => `
      <div class="list-item" style="pointer-events:none;">
        <div class="li-icon">◈</div>
        <div class="li-body">
          <div class="li-title">${esc(a.class_name || a.facility_slug || 'Tap')}</div>
          <div class="li-sub">${esc(a.doorway || '')} · ${fmtTime(a.tapped_at)}</div>
        </div>
      </div>`).join('') || '<div class="card dim-card"><div class="dim-text">No attendance records yet.</div></div>';
  }

  const act = $('#hubActivity');
  if (act) {
    act.innerHTML = (data.activity || []).slice(0, 10).map(l => `
      <div class="list-item" style="pointer-events:none;">
        <div class="li-icon">◉</div>
        <div class="li-body">
          <div class="li-title">${esc(l.action)}</div>
          <div class="li-sub">${esc(l.details || '')} · ${fmtTime(l.created_at)}</div>
        </div>
      </div>`).join('') || '<div class="card dim-card"><div class="dim-text">No activity yet.</div></div>';
  }

  // Timetable upload (bind once)
  const uploadCard = $('#hubUploadCard');
  if (uploadCard && !uploadCard.dataset.bound) {
    uploadCard.dataset.bound = '1';
    uploadCard.style.cursor = 'pointer';
    const fileInput = document.createElement('input');
    fileInput.type = 'file';
    fileInput.accept = '.ics,.csv,.txt';
    ...
    fileInput.addEventListener('change', async () => {
      if (!fileInput.files[0]) return;
      try {
        await CampusAPI.uploadTimetable(fileInput.files[0]);
        showToast('Timetable saved to your account');
        await loadMyData();
      } catch (e) { showToast(e.message); }
    });
  ...
  const saveBtn = $('#distSaveBtn');
  if (saveBtn) {
    saveBtn.onclick = async () => {
      const distance_km = Number($('#distKm')?.value);
      try {
        await CampusAPI.addDistance({
          distance_km,
          from_label: $('#distFrom')?.value || '',
          to_label: $('#distTo')?.value || '',
          note: $('#distNote')?.value || ''
        });
        if ($('#distKm')) $('#distKm').value = '';
        if ($('#distFrom')) $('#distFrom').value = '';
        if ($('#distTo')) $('#distTo').value = '';
        if ($('#distNote')) $('#distNote').value = '';
        showToast('Distance saved to your account');
        const fresh = await CampusAPI.distances();
        renderDistances(fresh.records, fresh.total_km);
      } catch (e) {
        showToast(e.message);
      }
    };
